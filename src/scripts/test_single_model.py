...
"""
Test a single model - phi-4-reasoning
"""
import asyncio
import logging
...
async def main():
    """Test only phi-4-reasoning"""
    tester = ModelManagerTester()
    
    # Get initial status
    status = tester.manager.get_status()
    logger.info("Testing phi-4-reasoning only")
    logger.info(f"Device: {status['device']}")
    
    # Test phi-4-reasoning
    if status["models"]["phi-4-reasoning"]["downloaded"]:
        results = await tester.test_model("phi-4-reasoning")
        ...
        logger.error("phi-4-reasoning not downloaded")
